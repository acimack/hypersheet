...
export const MainView = React.memo(function MainView() {
  ...
  // // function to check whether the character is a number
  ...
  const loadRootsFromDB = useCallback(async () => {
    ...
    if (rootsFromDB.success) {
      // filter out nodes with a number as the first character in id
      // rootsFromDB.payload?.filter()
  ...
  const [currentFirebaseUser, setCurrentFirebaseUser] = useState<any>({})
  const [showProfile, setShowProfile] = useState<boolean>(false)
  useEffect(() => {
    // if someone interacts with TreeView, hide profile
    setShowProfile(false)
  }, [selectedNode])
          ...
          <div className="content">
            ...
            {showProfile ? (
              <Profile
                ...
                setCurrentFirebaseUser={setCurrentFirebaseUser}
                currentUser={currentUser}
