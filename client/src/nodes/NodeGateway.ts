...
export const NodeGateway = {
   ...
   * anchor deletion will be handled by LinkGateway.deleteLinks().
   *
   * In this method we want to get the links associated with a node and delete those
   * links via LinkGateway.deleteLinks(). Once the links and associated orphan anchors
   * have been deleted, we can delete the node as we did in Assignment 1.
   *
   * TIPS:
   * You will likely need to make the following requests in this order:
   * - get anchors by nodeId (frontend AnchorGateway)
   * - get links by anchorIds (frontend LinkGateway)
   * - delete links and associated orphan anchors (frontend LinkGateway)
  ...
  deleteNode: async (nodeId: string): Promise<IServiceResponse<{}>> => {
    try {
      ...
      const anchorsResp: IServiceResponse<IAnchor[]> =
        await AnchorGateway.getAnchorsByNodeId(nodeId)
      ...
      const linksResp: IServiceResponse<ILink[]> = await LinkGateway.getLinksByAnchorIds(
        anchorIds
      )
