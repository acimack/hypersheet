 ...
 */
export type Extent = ITextExtent | IImageExtent | ISheetExtent
...
export function makeISheetExtent(row: number, column: number) {
  return {
    row: row,
    column: column,
    type: 'sheet' as 'sheet',
...
export function isISheetExtent(object: any): boolean {
  return (
    (object as ISheetExtent).type === 'sheet' &&
    typeof (object as ISheetExtent).row === 'number' &&
    typeof (object as ISheetExtent).column === 'number'
  )
}
